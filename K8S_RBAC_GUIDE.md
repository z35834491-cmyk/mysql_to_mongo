...
## 6. Perf 模块采样开关 RBAC（快速授权）

### 6.1 仅授权 trace-system 命名空间

```bash
kubectl apply -f k8s/rbac-perf-editor.yaml
kubectl auth can-i patch daemonsets.apps -n trace-system
```

### 6.2 跨命名空间授权（ClusterRole）

```bash
kubectl apply -f k8s/rbac-perf-editor-clusterrole.yaml
kubectl auth can-i patch daemonsets.apps -A
```

### 6.3 专用写权限账号（更安全）

```bash
kubectl apply -f k8s/perf-editor-sa.yaml
```

将 `trace-system:perf-editor` 的 kubeconfig 配置到 Django 的 ClusterConfig 用于“写操作”（采样开关），默认的 `default:perf-reader` 继续用于“只读”。
